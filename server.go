...
func initRouting() *mux.Router {
	...
	r.HandleFunc("/v1/agent/members/{id}", membersHandler)
	r.HandleFunc("/v1/ipxe/{id}", executeHandler)
	r.HandleFunc("/agent/{id}", executeAgentHandler)
	r.HandleFunc("/v1/ipxe/toggle/{id}", toggleHandler)
	r.HandleFunc("/v1/agent/toggle/{id}", toggleAgentHandler)
